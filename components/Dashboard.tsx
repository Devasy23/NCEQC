...
import { Activity, Settings, FileText, Edit2, Trash2, Plus, Users, Key } from 'lucide-react';
import { useTheme } from '../context/ThemeContext';
import { ModuleType, ViewState, Plugin, Template, Tenant, Credential } from '../types';
import { MOCK_PLUGINS, MOCK_TEMPLATES, MOCK_TENANTS, MOCK_CREDENTIALS, MODULES } from '../constants';
    ...
    const renderHome = () => (
        <div className="pb-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8">
                {MOCK_PLUGINS[activeModule].map((plugin) => (
    ...
    const renderTemplates = () => (
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {MOCK_TEMPLATES.filter(t => t.module === activeModule).map((template) => (
                        ...
                        <div className={`${getCardStyle(isDarkMode)} flex-row`}>
                            ...
                            <div className="p-4 flex-1">
                                ...
                                <div className="flex gap-3">
                                    <button onClick={() => onEdit(template, 'template')} className={`p-1.5 transition-colors ${isDarkMode ? 'text-gray-300 hover:text-white hover:bg-white/10 rounded' : 'border-2 border-black hover:bg-black hover:text-white'}`}>
                                        <Edit2 size={16} />
                                    </button>
                                    <button onClick={() => onDelete(template, 'template')} className={`p-1.5 transition-colors ${isDarkMode ? 'text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded' : 'border-2 border-black hover:bg-[#ff4444] hover:text-white'}`}>
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </div>
            ...
            <div className="space-y-5">
                {MOCK_TENANTS.map((tenant) => (
                    <TiltCard key={tenant.id}>
                        <div className={`${getCardStyle(isDarkMode)} h-auto p-0 flex-row items-stretch justify-between group ${isDarkMode ? 'hover:bg-white/10' : 'hover:bg-[#f0f0f0]'}`}>
                            ...
                            <div className={`flex ${isDarkMode ? 'border-l border-white/10' : 'border-l-2 border-black'}`}>
                                <button onClick={() => onEdit(tenant, 'tenant')} className={`px-6 transition-colors ${isDarkMode ? 'hover:bg-yellow-500/20 text-yellow-200 border-r border-white/10' : 'hover:bg-[#ffcc00] border-r-2 border-black text-black'}`}>
                                    <Edit2 size={24} />
                                </button>
                                <button onClick={() => onDelete(tenant, 'tenant')} className={`px-6 transition-colors ${isDarkMode ? 'hover:bg-red-500/20 text-red-300' : 'hover:bg-[#ff4444] hover:text-white text-black'}`}>
                                    <Trash2 size={24} />
                                </button>
                            </div>
                        </div>
                        ...
                        <div className={getCardStyle(isDarkMode)}>
                            <div className="p-6">
                                ...
                                <div className={`flex gap-3 pt-5 ${isDarkMode ? 'border-t border-white/10' : 'border-t-2 border-black border-dashed'}`}>
                                    <button onClick={() => onEdit(cred, 'credential')} className={`flex-1 py-2 text-sm font-black uppercase transition-colors ${isDarkMode ? 'bg-white/5 hover:bg-white/10 rounded border border-white/5' : 'border-2 border-black hover:bg-[#ffde59] hover:text-black'}`}>
                                        Edit
                                    </button>
                                    <button onClick={() => onDelete(cred, 'credential')} className={`flex-1 py-2 text-sm font-black uppercase transition-colors ${isDarkMode ? 'bg-white/5 hover:bg-red-500/20 hover:text-red-300 rounded border border-white/5' : 'border-2 border-black hover:bg-black hover:text-white'}`}>
                                        Delete
                                    </button>
                                </div>
                            </div>
